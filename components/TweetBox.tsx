...
function TweetBox({ setTweets }: Props) {
    ...
    // function to add image to tweet
    const addImageToTweet = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        // to prevent the page to refresh
        e.preventDefault();

        // if there is not image just return
        if (!imageInputRef.current?.value) return;
        // if every thing is clear just assign the value to the setImage state
    ...
    return (
        <div className='flex space-x-2 p-5'>
            <img className='h-14 w-14 mt-4 object-cover rounded-full'
            ...
            <div className='flex flex-1 items-center pl-2'>
                <form className='flex flex-1 flex-col'>
                    ...
                    {/* coditional render the image box if the button is clicked or not */}

                    {imageUrlBoxIsOpen && (
                        <form className='mt-5 flex rounded-lg bg-twitter/80 py-2 px-4'>
                            <input
                                ref={imageInputRef}
                                className='flex-1 bg-transparent p-2 text-white outline-none placeholder:text-white' type="text" name="" id="" placeholder='Enter Image URL....' />
                            <button
                                type='submit'
                    ...
                    {/* if there is an image just show the image */}
